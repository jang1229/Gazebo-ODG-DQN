    ...
    def step(self, action,episode_step):
        ...
        while data is None:
            try:
                ...
                datas = rospy.wait_for_message('/scan', LaserScan, timeout=5)
...
if __name__ == '__main__':
    rospy.init_node('DQN_main', anonymous=True)
            ...
            if done:
                ...
                if not last100Filled:
                    ...
                    print ("EP " + str(epoch) + " - " + format(episode_step + 1) + "/" + str(
                        steps) + " Episode steps   Exploration=" + str(
                        round(explorationRate, 2)) + " - Cumulated R: " + str(cumulated_reward))
                    ...
                    if (epoch)%100==0:
                        ...
                        with open("/home/sun/path/Distance/"+str(epoch)+"_Distance_DQN_Main.txt", "w") as file:
                            for i in range(epoch):
                                file.write(str(re_dis[i]) + ',')

                    if (epoch)%100==0:

                        with open("/home/sun/path/Distance/step"+str(epoch)+"_step_DQN_Main.txt", "w") as file:
                            for i in range(epoch):
                                file.write(str(episode_step_save[i]) + ',')

                    if (epoch)%100==0:

                        with open("/home/sun/path/re_path/"+str(epoch)+"_Accuracy_DQN_Main.txt", "w") as file:
                            for i in range(epoch):
                                file.write(str(y_data[i]) + ',')

                    if (epoch)%100==0:

                        with open("/home/sun/path/reward_dqn/"+str(epoch)+"_rewrad_DQN_Main.txt", "w") as file:
                            for i in range(epoch):
                                file.write(str(reward_data[i+1]) + ',')

                    if (epoch)%100==0:

                        with open("/home/sun/path/speed_time/speed/"+str(epoch)+"_speed_DQN_Main.txt", "w") as file:
                            for i in range(epoch):
                                file.write(str(speed_data[i+1]) + ',')

                    if (epoch)%100==0:

                        with open("/home/sun/path/speed_time/time/"+str(epoch)+"_time_DQN_Main.txt", "w") as file:
                            for i in range(epoch):
                    ...
                    if (epoch)%100==0:
                        deepQ.saveModel(path2 + str(epoch) +'DQN_Main'+ '.h5')
