...
class env():
   ...
   '''
        ...
        return np.asarray(state), reward, done, {},xlist,ylist,datas.angle_min
...
if __name__ == '__main__':
    ...
    env = env()
    outdir = '/tmp/ddqn/'
    path = '/home/sun/path/Network Weights/'
    ...
    if not continue_execution:
        ...
        deepQ = ODG_MAX.DeepQ(network_inputs, network_outputs, memorySize, discountFactor, learningRate, learnStart)
       ...
       # plt.ylabel("error_average", fontsize=13)
        ...
        while not done:
           ...
           # print "------"
            ...
            action = deepQ.selectAction(qValues, explorationRate, MinODG)
            #print action
             #reset
            ...
            newObservation, reward, done, info,xlist,ylist,MinODG = env.step(action,episode_step)
               ...
               # print time_data
                ...
                if not last100Filled:
                    ...
                    if (epoch)%100==0:
                        ...
                        with open("/home/sun/path/Distance/"+str(epoch)+"_Distance_DQN.txt", "w") as file:
                            for i in range(epoch):
                                file.write(str(re_dis[i]) + ',')

                    if (epoch)%100==0:

                        with open("/home/sun/path/Distance/step"+str(epoch)+"_step_DQN.txt", "w") as file:
                            for i in range(epoch):
                    ...
                    if (epoch)%100==0:
                        ...
                        with open("/home/sun/path/speed_time/speed/"+str(epoch)+"_speed_DQN.txt", "w") as file:
                            for i in range(epoch):
                                file.write(str(speed_data[i+1]) + ',')

                    if (epoch)%100==0:

                        with open("/home/sun/path/speed_time/time/"+str(epoch)+"_time_DQN.txt", "w") as file:
                            for i in range(epoch):
