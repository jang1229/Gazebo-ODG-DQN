...
class env():
    ...
    def step(self, action,episode_step):
        ...
        done,goal_reward = self.calculate_observation(data, point)
        state = datas.ranges
        self.when = 0

        max_ang_speed = 0.7
        ang_vel = (datas.angle_min-60)*max_ang_speed*0.01 #from (-0.33 to + 0.33)
   ...
   '''
    ...
    def reset(self):
        ...
        done= self.calculate_observation(data, point)
...
if __name__ == '__main__':
    rospy.init_node('DQN_ODG_path', anonymous=True)
